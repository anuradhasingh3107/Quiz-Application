...
class Authenticator extends Component { // constructor
    constructor(props) {
        super(props);
    ...
    processResponse(res) {
        if (res.isValid) {
            let tempState = {
                role: res.userInfo[0].Role
            };
            store.dispatch(createAction(Constants.SET_USER_DATA, tempState));
            console.log("User authenticated");
            this.props.history.push("/user");
        } else { // this.setState(this.resetState());
        }
    ...
    render() {
        return (<div className="container-fluid">
            ...
            <form onSubmit={
                ...
                className=".btn-flat">
                <input className="form-control form-control-lg" size="lg" id="userName" type='text' placeholder="userName"
                    value={
                        this.state.userName
                    }
                    onChange={
                        this.handleChangeFor('userName')
                    }/>
                <br/>
                <input className="form-control form-control-lg" id="passWord" type='password' placeholder="password"
                    ...
                    onChange={
                        this.handleChangeFor('passWord')
                    }/>
                <br/>
                <button type="submit" className="btn btn-primary"
                    disabled={
                        this.state.loginDisability
                }>
                    Login
                </button>
                <br/><br/><br/>
